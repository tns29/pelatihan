...
<section class="content">
    <div class="container-fluid">
        <div class="card mx-2 elevation-1 p-3 w-75">
            <form action="/service-detail/{{ $data->id }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="row mx-2">
                    <div class="col-lg-6 col-md-6 col-sm-6 mt-2">
                        ...
                        <select class="form-control @error('training_id')is-invalid @enderror" name="training_id" id="training_id">
                            ...
                            @foreach ($dataTraining as $item)
                                <option value="{{ $item->id }}" {{ $data->training_id == $item->id ? 'selected':''}}> » &nbsp; {{ $item->title }}</option>
                            @endforeach
                        </select>
                        @error('training_id')
                    ...
                    <div class="col-lg-5 col-md-5 col-sm-5 mt-2">
                        <div class="form-check mt-1">
                            ...
                            <input class="form-check-input mt-5" type="checkbox" {{ $data->is_active == 'Y' ? 'checked' : '' }} value="Y" name="is_active" id="is_active" style="width: 1.3rem; height: 1.3rem; top:-1rem; left: 2.5rem;">
                    ...
                    <div class="col-lg-6 col-md-6 col-sm-6 mt-2">
                        ...
                        <div class="card img-bordered ml-5 p-2">
                            @if ($data->image)
                                <img id="blah" src="{{ asset('/storage').'/'.$data->image }}" alt="preview" style="height: 250px;"/>
                            @else
